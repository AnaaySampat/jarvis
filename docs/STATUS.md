...
0c. **Re-run the speed benchmark** after re-enabling accessibility: stopwatch start, stop+reset,
   Settings search from a fresh Settings, Spotify play. The model (not the mechanics) now
   dominates: gemma-4-26b image steps take 4–8s and flash-lite routes hit 429/503 often;
   a faster first route for the operator is the next lever (routing is the owner's call).
