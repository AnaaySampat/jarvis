...
0c. **Make multi-screen tasks cheaper.** The live runs were correct but slow (21–82s): Samsung's
   Settings search is a separate package (`settings.intelligence`) the operator mistook for
   not-Settings, and Gemma's image steps take 4–8s against 2s without. Candidates: tell the
   model a sub-package is still the same app; skip the screenshot when the list is rich.
